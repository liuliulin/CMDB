...
class MyUser(AbstractBaseUser):
    ...
    group = models.ManyToManyField("MyGroup", blank=True, verbose_name="部门或组")
    remarks = models.TextField(u"备注", blank=True, null=True, default=None)
...
class MyGroup(models.Model):
    ...
    owner = models.ForeignKey("MyUser", default='admin', verbose_name="负责人")
...
class Asset(models.Model):
    ...
    manager = models.ForeignKey(MyUser, null=True, blank=True, verbose_name="资产管理员")
    ...
    approved_by = models.ForeignKey(MyUser, null=True, blank=True, verbose_name="批准人", related_name="approved_by")
...
class CPU(models.Model):
    ...
    asset = models.OneToOneField("Asset")  # 单设备cpu都是一样的，不需要建立多个cpu数据，一条就可以，使用一对一。
...
class EventLog(models.Model):
    ...
    user = models.ForeignKey(MyUser, blank=True, null=True, verbose_name="事件执行人", on_delete=models.SET_NULL)
    # 自动更新资产数据时没有执行人
    memo = models.TextField("备注", blank=True, null=True)
