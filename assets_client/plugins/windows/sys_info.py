...
def collect():
    ...
    host = wmi.WMI().Win32_computerSystem()[0].model
    k, v = host.split()
    if k == "PowerEdge":
        data.update(get_disk_info_dell())
    else:
        data.update(win32obj.get_disk_info_wmi())
    # 最后返回一个数据字典
    return data
...
def get_disk_info_dell():
    """
    主机需要安装 Dell_DSET 软件，并且只能在管理员下运行
    :return:
    """
    from subprocess import Popen, PIPE
    raw = Popen('omreport storage pdisk controller=0', stdout=PIPE, stderr=PIPE, shell=True)
            ...
            for i in item:
                ...
                if len(data) == 2:
                    ...
                    if key.strip() == 'Capacity':
                        disk_item_dic["capacity"] = v.split()[0].strip()
                    if key.strip() == 'Vendor ID':
                        disk_item_dic["manufacturer"] = v.strip()
                    if key.strip() == 'Serial No.':
                        disk_item_dic['sn'] = v.strip()
                    if key.strip() == 'Product ID':
                        disk_item_dic['model'] = v.strip()
                    if key.strip() == 'ID':
                        disk_item_dic['slot'] = v.strip()
                    if key.strip() == 'Bus Protocol':
                        disk_item_dic['iface_type'] = v.strip()
